...
public:
    explicit ServerTextSessionCache(
    ...
    PreparedPromptFactory<Model> prepare_prompt = {})
{
    ...
    auto sample_first_token = [&]() {
        ...
        if (transformed_prompt) {
            auto hidden = server_hidden_forward_prepared_chunked(
...
int mfq::cuda::run_decode(int argc, char ** argv) {
    ...
    try {
        ...
        if (server_mode) {
            ...
            model_capabilities.text =
                server_components.graph.has_component("text") &&
                server_components.plan.backbone !=
            ...
            model_capabilities.video_input =
                component_state.vision_available &&
                !server_components.grid_vision.has_value();
            const bool composite_loaded =
                server_components.minicpmo.has_value();
            model_capabilities.audio_input = composite_loaded &&
                server_components.graph.has_component("audio_input");
            ...
            const auto& runtime_assets = *server_model.source;
            if (runtime_assets.has_asset(mfq::cuda::kTokenizerGgufAsset)) {
                ...
                server_config.tokenizer_model = tokenizer_model;
            } else {
                throw std::runtime_error(
                    "model server requires a tokenizer GGUF");
            }
            ...
            server_config.runtime_profile = resolve_mfq_runtime_profile(
                model_path,
                server_components.graph.architecture,
            ...
            } else if (server_components.grid_vision) {
                multimodal_generate =
                    [&](const std::vector<int64_t>& prompt,
                        const MfqVisionInput& vision,
                        const MfqSamplingParams& sampling,
            ...
            [&server_components, &continuous_batcher, &model_mutex] {
                size_t free_bytes = 0;
                ...
                if (mtp_available && lock.owns_lock() &&
                    server_components.mtp) {
                    const auto& mtp_stats = server_components.mtp->last_stats;
                    result.emplace_back(
