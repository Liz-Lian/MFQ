...
struct Affine {
  ...
  static Affine load(const MfqContainer &model,
    ...
    auto bias_name = grid_vision_canonical_name(bias_role, block_index);
    std::optional<array> bias;
    ...
    Affine result{
        MlxLinear::load(
            model, grid_vision_canonical_name(weight_role, block_index)),
        std::move(bias),
    };
...
struct LayerNorm {
  ...
  static LayerNorm load(const MfqContainer &model,
    ...
    return {
        required_vector(
            model, grid_vision_canonical_name(weight_role, block_index), width),
        required_vector(
            model, grid_vision_canonical_name(bias_role, block_index), width),
        eps,
    };
  }
...
MlxGridVisionEncoder::load(const MfqContainer &model,
  ...
  auto patch_weight = dense_tensor(
      model, grid_vision_canonical_name(GridVisionTensorRole::patch_weight));
  ...
  auto patch_bias = required_vector(
      model, grid_vision_canonical_name(GridVisionTensorRole::patch_bias), hidden);
  auto position_weight = dense_tensor(
      model, grid_vision_canonical_name(GridVisionTensorRole::position_weight));
  ...
  for (std::size_t index = 0; index < static_cast<std::size_t>(config.depth);
       ++index) {
    VisionBlock block{
        LayerNorm::load(model, GridVisionTensorRole::block_norm1_weight,
        ...
        LayerNorm::load(model, GridVisionTensorRole::block_norm2_weight,
                        GridVisionTensorRole::block_norm2_bias, hidden,
                        static_cast<float>(config.layer_norm_eps), index),
        Affine::load(model, GridVisionTensorRole::block_mlp_up_weight,
                     GridVisionTensorRole::block_mlp_up_bias, index),
        Affine::load(model, GridVisionTensorRole::block_mlp_down_weight,
...
MlxQwenVisionTower MlxQwenVisionTower::load(const MfqContainer &model,
  ...
  auto merger_norm =
      LayerNorm::load(model, GridVisionTensorRole::merger_norm_weight,
                      GridVisionTensorRole::merger_norm_bias, hidden,
                      static_cast<float>(config.layer_norm_eps));
  auto merger_up =
      Affine::load(model, GridVisionTensorRole::merger_mlp_up_weight,
                   GridVisionTensorRole::merger_mlp_up_bias);
  auto merger_down =
      Affine::load(model, GridVisionTensorRole::merger_mlp_down_weight,
