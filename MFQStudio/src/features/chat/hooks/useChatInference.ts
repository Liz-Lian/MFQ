...
import type { SessionMode, SamplingParams } from '../../../api';
...
export function useChatInference(mode: SessionMode) {
  ...
  const reasoningValues = runtime?.chat_template_capabilities?.reasoning_effort?.values ?? [];
  ...
  return {
    ...
    availableModelNames: runtimeModelNames(models, instances),
