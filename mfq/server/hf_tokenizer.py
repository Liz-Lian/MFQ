...
def _fingerprint_payloads(payloads: tuple[tuple[str, bytes], ...]) -> str:
    ...
    digest.update(b"mfq-hf-tokenizer-gguf-v8\0")
    ...
    digest.update(DEEPSEEK_V4_CHAT_TEMPLATE.encode("utf-8"))
    digest.update(DEEPSEEK_V41_CHAT_TEMPLATE.encode("utf-8"))
...
def _resolve_chat_template(
    ...
    identities = tuple(
        ...
        for value in (text_config.get("model_type"), config.get("model_type"))
        if isinstance(value, str) and value.strip()
    )
    if any(
        identity == "deepseek_v41" or identity.startswith("deepseek_v41_")
        for identity in identities
    ):
        return DEEPSEEK_V41_CHAT_TEMPLATE
    if any(
        identity == "deepseek_v4" or identity.startswith("deepseek_v4_")
        for identity in identities
    ):
