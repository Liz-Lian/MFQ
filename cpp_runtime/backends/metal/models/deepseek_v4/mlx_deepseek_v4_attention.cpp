...
void MlxDeepseekV4PoolState::restore_speculative_snapshot(
    ...
    // mlx_dsa_decode_pool_step produces new state arrays. Pool writes are
...
void MlxDeepseekV4PoolState::prefill(
    ...
    if (windows > 0) {
        ...
        auto compressed = mlx_dsa_compress(
            grouped_kv,
            grouped_gate,
            ape,
            norm,
...
void MlxDeepseekV4LayerState::restore_speculative_snapshot(
    ...
    local_ = mlx_cache_write_inplace(
        local_,
        snapshot.local_,
        rows);
    if (main_) {
...
MlxDeepseekV4Attention MlxDeepseekV4Attention::load(
    ...
    auto compressed = mlx_yarn_tables(
        checked_int(
            config.qk_rope_head_dim,
            "rotary dimension"),
        max_context,
...
array MlxDeepseekV4Attention::operator()(
    ...
    if (tokens == 1) {
        ...
        if (config.fast_attention()) {
            ...
            direct_decode = mlx_dsa_sparse_decode_attention(
                mlx::core::transpose(
                    q,
                    {0, 2, 1, 3}),
                state.local_,
            ...
            plan = mlx_dsa_build_decode_plan(
                topk,
                mlx::core::full(
                    Shape{batch},
                    pos0 + 1,
