...
mfq_tensor_backend::Tensor rope_table_bf16_cuda(
    mfq_tensor_backend::Tensor x,
    mfq_tensor_backend::Tensor pos,
    mfq_tensor_backend::Tensor cos,
    mfq_tensor_backend::Tensor sin,
...
void run_rope(
    ...
    if (options.dtype != "f32" && options.dtype != "bf16") {
        usage_error("rope supports --dtype f32 or bf16");
    }
    ...
    auto input = mfq::cuda::randn({options.rows, options.width}, fp32)
        .to(scalar_type(options.dtype)).contiguous();
    ...
    auto reference = mfq::cuda::tensor(expected_values, fp32)
        .reshape({options.rows, options.width})
        .to(scalar_type(options.dtype)).contiguous();

    auto call = [&] {
        return options.dtype == "bf16"
            ? rope_table_bf16_cuda(input, pos, cos, sin, rotary_width)
            : rope_table_cuda(
    ...
    correctness.tolerance = options.dtype == "bf16" ? 3.0e-2 : 2.0e-6;
