...
Options parse_options(int argc, char** argv) {
    ...
    if (first == "--help" || first == "-h") {
        std::cout
            << "usage: mfq-cuda-common-ops-bench "
               "<acc|acc-rms-norm|silu-mul|rms-norm|rope|embedding|kv-cache> [options]\n";
        std::exit(0);
    }
    ...
    if (options.operation != "acc" && options.operation != "acc-rms-norm" &&
        ...
        options.operation != "rope" && options.operation != "embedding" &&
        options.operation != "kv-cache") {
...
int main(int argc, char** argv) {
    try {
        ...
        } else if (options.operation == "embedding") {
            mfq::bench::run_embedding(options, gpu, properties);
        } else if (options.operation == "kv-cache") {
            mfq::bench::run_kv_cache(options, gpu, properties);
        } else {
